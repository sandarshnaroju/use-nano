...
export function addRulesInWebpack(libRules, repoName) {
  try {
    ...
    const stringTillResolve = exportModules.slice(
      0,
      exportModules.indexOf("resolve:")
    );
    const stringAfterResolve = exportModules.slice(
      exportModules.indexOf("resolve:")
    );
    const moduleExportsToLastExistingRules = stringTillResolve.slice(
      0,
      stringTillResolve.lastIndexOf("],") - 1
    );
...
export function addAliasToWebPack(aliasObject, repoName) {
  ...
  const stringTillAlias = stringTillExtensions.slice(
    0,
    stringTillExtensions.lastIndexOf("},") - 1
  );
  let stringifiedAlias = "";
  Object.entries(aliasObject).forEach(([key, value]) => {
    if ((value as string).includes("resolve")) {
