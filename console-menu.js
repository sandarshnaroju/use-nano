  ...
  return new Promise((resolve, reject) => {
    ...
    var handleMenuKeypress = (ch, key) => {
      ...
      if (isEnter(key)) {
        ...
        selection =
          items.find((item) => item.hotkey && item.hotkey === ch) ||
          items.find(
            (item) =>
              item.hotkey && item.hotkey.toLowerCase() === ch.toLowerCase()
          );
      }
      ...
      if (selection || isCancelCommand(key)) {
        ...
        newIndex = options.pageSize
          ? Math.max(0, selectedIndex - options.pageSize)
          : 0;
        while (newIndex < count && items[newIndex].separator) newIndex++;
      } else if (isPageDownCommand(key) && selectedIndex < count - 1) {
        newIndex = options.pageSize
          ? Math.min(count - 1, selectedIndex + options.pageSize)
          : count - 1;
        while (newIndex >= 0 && items[newIndex].separator) newIndex--;
      ...
      if (newIndex !== null && newIndex >= 0 && newIndex < count) {
        ...
        if (selectedIndex < scrollOffset) {
          scrollOffset = isPageUpCommand(key)
            ? Math.max(0, scrollOffset - options.pageSize)
            : selectedIndex;
        } else if (
          options.pageSize &&
          selectedIndex >= scrollOffset + options.pageSize
        ) {
          scrollOffset = isPageDownCommand(key)
            ? Math.min(
                count - options.pageSize,
                scrollOffset + options.pageSize
              )
  ...
  if (options.header) {
    process.stdout.write(
      prefix +
        (options.border ? " " : "") +
        options.header +
        repeat(" ", width - options.header.length) +
        suffix +
        os.EOL
    );
    if (options.border) {
