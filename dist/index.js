...
import fs from "fs";
import { themeText, commonUrl } from "./Constants.js";
...
import { askUserInfoToGenerateKeyStoreFile, generateAabWhenKeyStoreExists, generateApkWhenKeyStoreExists, generateDebugAabWhenKeyStoreExists, generateDebugApkWhenKeyStoreExists, } from "./apprelease/AndroidRelease.js";
...
const npmInstallRequiredPackagesInRNProject = ({ repoName, appId, appSecret, isSyncFunctionalityRequired = false, nanoversion = null, }) => {
...
const setUpANewProjectWithDefaultLoadingScreen = ({ repoName, appId = null, appSecret = null, nanoversion, reactNativeVers, }) => {
...
const setUpANewMinimalProject = async ({ repoName, appId = null, appSecret = null, nanoversion, reactNativeVers, }) => {
    const RNversionString = reactNativeVers != null ? `--version ${reactNativeVers}` : "";
    const reactnativeinstall = `npx react-native init ${repoName} ${RNversionString}`;
    execSync(`${reactnativeinstall}`);
    if (fs.existsSync(path.join(repoName, "/App.tsx"))) {
        fs.unlinkSync(path.join(repoName, "/App.tsx"));
...
const createProjectWithSyncEnabled = ({ projectName, nanoversion, reactNativeVers, }) => {
    inquirer
        .prompt([
    ...
    switch (command) {
        case "init":
            ...
            inquirer
                ...
                .then((answers) => {
                if (argv2 != null) {
                    reactNativeVersion = argv2["react-native-version" /* COMMAND_ARGUMENTS.REACT_NATIVE_VERSION */];
                    realmVersion = argv2["realm" /* COMMAND_ARGUMENTS.REALM */];
                    reanimatedVersion =
                        argv2["react-native-reanimated" /* COMMAND_ARGUMENTS.REACT_NATIVE_REANIMATED */];
                    rnNanoVersion = argv2["nano-version" /* COMMAND_ARGUMENTS.NANO_VERSION */];
                }
