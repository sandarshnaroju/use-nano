...
export const changeAppUrlAppIdAppSecretInExistingProject = () => {
    const argv = yargs(hideBin(process.argv)).argv;
    if (argv && argv["app-id"] && argv["app-secret"] && argv["app-url"]) {
        const isCliendIdReplace = replaceLineInFileSync("nano.config.js", "export const CLIENT_ID", `export const CLIENT_ID = "${argv["app-id"]}"`);
        if (!isCliendIdReplace) {
            console.log("Error in replacing client id");
            return;
        }
        const isClientSecretReplace = replaceLineInFileSync("nano.config.js", "export const CLIENT_SECRET", `export const CLIENT_SECRET = "${argv["app-secret"]}"`);
        if (!isClientSecretReplace) {
            console.log("Error in replacing client secret");
            return;
        }
        const isAppUrlReplace = replaceLineInFileSync("nano.config.js", "export const APP_URL", `export const APP_URL = "${argv["app-url"]}"`);
        if (!isAppUrlReplace) {
